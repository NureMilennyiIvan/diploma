...
import {LiquidityPoolTestingEnvironment} from "./helpers"
import {
...
export const ammsConfigTests = (
    liquidityPoolTestingEnvironment: LiquidityPoolTestingEnvironment,
    ammsConfigsManagerAddress: ProgramDerivedAddress,
    ammsConfigAddress: ProgramDerivedAddress
) => {
    describe("\nAmmsConfig tests", () => {
        const { program, rpcClient, rent, headAuthority, owner, ammsConfigsManagerAuthority, user } = liquidityPoolTestingEnvironment;
        ...
        it("Initialize AmmsConfig by head authority", async () => {
            ...
            const ammsConfigAccount: Account<AmmsConfig> = await program.fetchAmmsConfig(rpcClient.rpc, ammsConfigAddress[0]);
            const ammsConfigsManagerAccountAfter: Account<AmmsConfigsManager>  = await program.fetchAmmsConfigsManager(rpcClient.rpc, ammsConfigsManagerAddress[0]);
        ...
        it("Initialize AmmsConfig by authority", async () => {
            ...
            const testAmmsConfigAddress = await getProgramDerivedAddress({
                programAddress: liquidityPoolTestingEnvironment.program.LIQUIDITY_POOL_PROGRAM_ADDRESS,
            ...
            const testAmmsConfigAccount: Account<AmmsConfig> = await program.fetchAmmsConfig(rpcClient.rpc, testAmmsConfigAddress[0]);
            const ammsConfigsManagerAccountAfter: Account<AmmsConfigsManager> = await program.fetchAmmsConfigsManager(rpcClient.rpc, ammsConfigsManagerAddress[0]);
        ...
        it("Unauthorized attempt to update AmmsConfig fee authority should fail", async () => {
            const input: UpdateAmmsConfigFeeAuthorityInput = {
                ...
                ammsConfigsManager: ammsConfigsManagerAddress[0],
                ammsConfig: ammsConfigAddress[0],
                newFeeAuthority: liquidityPoolTestingEnvironment.headAuthority.address
            };
