...
import {
    ...
    getTransferCheckedInstruction as getTransferChecked22Instruction
...
export const transferTokens = async (rpcClient: RpcClient, source: Address, destination: Address, mint: Address, sourceAuthority: KeyPairSigner, amount: bigint, decimals: number): Promise<Signature> => {
    const instructions = [
        getTransferCheckedInstruction({
            source,
            mint,
    ...
    return await pipe(
        await createTransaction(rpcClient, sourceAuthority, instructions),
        (tx) => signAndSendTransaction(rpcClient, tx)
    );
};

export const transferTokens22 = async (rpcClient: RpcClient, source: Address, destination: Address, mint: Address, sourceAuthority: KeyPairSigner, amount: bigint, decimals: number): Promise<Signature> => {
    const instructions = [
        getTransferChecked22Instruction({
            source,
            mint,
            destination,
            authority: sourceAuthority,
